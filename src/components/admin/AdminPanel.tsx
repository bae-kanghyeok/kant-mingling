...
import { GMRemote } from "./GMRemote";

const sessionCommands = new Set(["assign-teams", "publish-teams", "start-game1", "publish-next-block", "end-session", "transfer-host", "update-global-settings", "swap-seats", "upsert-participant", "remove-participant", "mark-attendance", "request-rotation", "cancel-rotation"]);
  ...
  return <Modal title={gmMode ? "GM 리모컨" : "관리자"} onClose={onClose} wide className="admin-panel">
    ...
    {tab === "progress" && <>
      ...
      {admin.isHost && <section className="host-tools"><h3>{gmMode ? "전체 진행 · 총괄 GM" : "전체 진행 · 호스트"}</h3>{gmMode && state.event.rotationRequested && <p className="notice">준비 완료 {admin.teams.filter(item => item.rotationReady).length} / {admin.teams.length}조 · 각 조가 현재 판과 대화를 마치면 새 조를 공개할 수 있어요.</p>}{state.event.phase === "BREAK" && !admin.nextBlockPlan && <p className="notice">명단과 전체 설정을 확인한 뒤 저장해주세요. 조 수와 운영진 배정이 맞으면 다음 자리 배정안이 만들어져요.</p>}{admin.nextBlockPlan && <div className="plan-list">{admin.nextBlockPlan.map((item) => <p key={item.teamKey}><strong>{item.teamKey}조</strong> {item.members.join(" · ")}</p>)}</div>}
        ...
        {!gmMode && can("start-game1") && <label className="check-row"><input type="checkbox" checked={forceStart} onChange={(event) => setForceStart(event.target.checked)} />미완료 참가자를 확인했고, 현재 인원으로 시작</label>}
        <div className="action-grid">{button("assign-teams")}{button("publish-teams")}{button("start-game1", { force: forceStart })}{button("request-rotation")}{button("cancel-rotation")}{button("publish-next-block")}{button("end-session", !gmMode && state.event.currentBlock < 3 ? { emergency: true, confirm: true, confirmEmergency: true } : { confirm: true })}</div>
