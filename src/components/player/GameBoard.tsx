...
function Ensemble({ state, game, send, busy }: { state: PublicState; game: PublicGame; send: SendAction; busy: boolean }) {
  ...
  if (ensemble.stage === "SHARE") return <section className="panel ensemble-panel"><p className="eyebrow">Ensemble</p><h2>{isSharer ? `Data ${number(ensemble.triggerCardNo)} 내용을 팀에 공유해주세요.` : `${ensemble.sharer.displayName}님이 Data ${number(ensemble.triggerCardNo)}를 공유하고 있어요.`}</h2><p className="muted">{isSharer ? "공유했으면 아래 버튼을 눌러주세요." : "곧 Ensemble이 시작됩니다."}</p>{state.allowedActions.includes("ensemble-shared") && <button className="button primary full" disabled={busy || state.team?.paused} onClick={() => command("ensemble-shared")}>공유했어요</button>}</section>;
...
export function GameBoard({ state, game, send, busy, initialGuessOpen = false, initialWrongFeedback = false }: {
  ...
  return <>
    <SectionTitle label={`Whose Data? · ${state.team?.key ?? ""}조`} title={`Game ${game.gameNo}${game.cards.length === 1 ? " 시작" : ""}`} description="우리 조의 Data Owner를 찾아보세요." />
    <div className="game-strip"><span>Data <strong>{number(game.cards.length)}</strong></span><span>이번 Turn Lead: <strong>{game.turnLead?.displayName ?? "—"}</strong></span></div>
    ...
    {game.phase === "TURN" && isLead && <section className="turn-actions"><p className="eyebrow">이번 Turn Lead는 당신입니다.</p><p className="muted">팀원들과 이야기한 뒤 다음 행동을 선택해주세요.</p>
      {wrongAt === game.cards.length && <div className="notice error-tone"><strong>아직 정답이 아니에요</strong>{!game.exhausted && <p>새로운 Data를 확인한 뒤 다시 추리해보세요.</p>}</div>}
      {game.exhausted && <p className="notice">모든 Data가 공개되었습니다.</p>}
      <div className="action-grid">{!game.exhausted && <button className="button secondary" disabled={busy || !state.allowedActions.includes("more-data") || state.team?.paused} onClick={more}>{wrongAt === game.cards.length ? "다음 Data 보기" : "Data 하나 더 보기"}</button>}<button className="button primary" disabled={busy || !game.guess?.enabled || !state.allowedActions.includes("guess") || state.team?.paused} onClick={() => setGuessOpen(true)}>추리하기</button></div>
      {!game.guess?.enabled && !game.guess?.locked && game.gameNo > 1 && game.cards.length < 3 && <p className="small muted center">Data가 3개 공개되면 추리할 수 있어요.</p>}
    </section>}
